...
const InvoiceDetailView: React.FC<InvoiceDetailViewProps> = ({ invoice }) => {
  ...
  const subtotal = invoice.tax_amount
    ? (invoice.total_amount || 0) - (invoice.tax_amount || 0)
  ...
  const [pdfUrl, setPdfUrl] = useState<string | null>(
    invoice.supabase_pdf_url || null
  );
  const { generatePDF, isGenerating: loading } = usePDF();
  ...
  const formattedInvoiceNumber = useMemo(() => {
    try {
      ...
      if (invoice.date_of_invoice) {
        const invoiceDate = new Date(invoice.date_of_invoice);
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-900">
              {formatDate(invoice.date_of_invoice)}
            </p>
          </div>
          <div>
