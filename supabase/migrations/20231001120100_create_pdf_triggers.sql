...
BEGIN
  IF (TG_OP = 'INSERT') OR 
    ...
    INSERT INTO gl_pdf_generation_queue(document_type, document_id)
...
DROP TRIGGER IF EXISTS invoice_pdf_trigger ON gl_invoices;
CREATE TRIGGER invoice_pdf_trigger
AFTER INSERT OR UPDATE ON gl_invoices
FOR EACH ROW
EXECUTE FUNCTION queue_invoice_pdf_generation();

DROP TRIGGER IF EXISTS estimate_pdf_trigger ON gl_estimates;
CREATE TRIGGER estimate_pdf_trigger
AFTER INSERT OR UPDATE ON gl_estimates
FOR EACH ROW
EXECUTE FUNCTION queue_estimate_pdf_generation();

DROP TRIGGER IF EXISTS purchase_order_pdf_trigger ON gl_purchase_orders;
CREATE TRIGGER purchase_order_pdf_trigger
AFTER INSERT OR UPDATE ON gl_purchase_orders
FOR EACH ROW
EXECUTE FUNCTION queue_purchase_order_pdf_generation();
