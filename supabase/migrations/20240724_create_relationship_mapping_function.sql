...
AS $$
DECLARE
  v_result JSONB := jsonb_build_object('mapped_count', 0, 'tables', '{}');
...
BEGIN
  ...
  LOOP
    ...
    IF v_column_record.column_name LIKE 'rowid_%' THEN
      ...
      v_rel_column := 'sb_' || substring(v_column_record.column_name FROM 7);
    ELSE
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'tables_processed', jsonb_object_agg(key, value->>'mapped_count') 
      FROM jsonb_each(v_result)
      WHERE jsonb_typeof(value) = 'object' AND value ? 'mapped_count',
    'details', v_result
  );
END;
$$;
...
BEGIN
  ...
  LOOP
    ...
    IF NEW.column_record.column_name IS NULL THEN
      CONTINUE;
    END IF;
    ...
    ', v_target_table) INTO v_target_id USING NEW.column_record.column_name;
